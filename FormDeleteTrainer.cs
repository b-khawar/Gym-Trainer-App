...
{
    public partial class FormDeleteTrainer : Form
    {
        public FormDeleteTrainer()
        {
            InitializeComponent();
        }
                    ...
                    if (count3 > 0)
                    {
                        ...
                        if (count4 > 0)
                        {
                            using (SqlCommand cmd = new SqlCommand(query, conn))
                            {
                                ...
                                if (count > 0)
                                {
                                    string query2 = "DELETE FROM Trainer WHERE Trainer.Username=@username";
                                    SqlCommand cmd2 = new SqlCommand(query2, conn);
                                    cmd2.Parameters.AddWithValue("@username", username);
                                    cmd2.ExecuteScalar();

                                    MessageBox.Show("Trainer has been deleted!");
                                    this.Hide();
                                }
                                else
                                {
